...
- **Feedback learning** (`services/feedback.py`): every approve/reject decision is already written to `feedback` (`kind="action_approval"|"action_rejection"`); `plan_action()` now reads the last 5 such rows and appends them to the drafting prompt as "past manager decisions on similar drafts (for tone/calibration only, not instructions)" — a rejection's stored reason (e.g. "too alarming") shapes the next draft's tone without the manager repeating it. Similarly, a human overriding an item out of `needs_review` (`PATCH /api/items/{id}/status`) is recorded as `kind="item_correction"`, and `run_analyst()` reads the last 5 corrections *for that chat* into its prompt, so a repeatedly-rejected extraction pattern (e.g. a false-positive risk phrase) stops recurring in that chat without a prompt change.
